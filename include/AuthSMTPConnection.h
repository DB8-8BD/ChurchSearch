...
public:
	...
	{
		int nBufLen = 0;
		LPCSTR lpszHostName = m_pszHostName;
		if (!lpszHostName)
			return FALSE;
		...
		CSocketAddr address;
		if (address.FindAddr(CA2T(lpszHostName),m_iPort,0,PF_UNSPEC,SOCK_STREAM,0))
			return FALSE;
				...
				if (bRet)
				{
					if (nDestLen >= nLen)
					{
						...
						if (bRet)
						{
							int nDestLen = sizeof(m_uzBuf);
							if (iAuth == 0)
								bRet = Base64Encode((const BYTE *)m_pszName,m_nNameLen,(LPSTR)&m_szBuf,&nDestLen);
							else
								bRet = Base64Encode((const BYTE *)m_pszPwd,m_nPwdLen,(LPSTR)&m_szBuf,&nDestLen);
							if (bRet)
							{
#if _SECURE_ATL
