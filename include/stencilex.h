...
public:
    ...
    bool GetNextMethodInfo(LPCSTR *ppszStart, LPSTR szMethodName, LPSTR szHandlerName, DWORD *pdwNot) throw()
    {
        *pdwNot = 0;
        *szMethodName = '\0';
        *szHandlerName = '\0';

        LPCSTR szStart = *ppszStart;
        ...
        pszTagEnd = pszTag-1;
        pszTag = szTag;
		DWORD dwMethodLen = strlen(szMethodName);
		DWORD dwHandlerLen = strlen(szHandlerName);
        // get the handler and method names
        if (HTTP_SUCCESS != GetHandlerAndMethodNames(pszTag, pszTagEnd, szMethodName, dwMethodLen, szHandlerName, dwHandlerLen))
        {
    ...
    DWORD ParseIfNot(LPCSTR szTokenStart, LPCSTR szTokenEnd, DWORD *pBlockStack, DWORD *pdwTop) throw()
    {
