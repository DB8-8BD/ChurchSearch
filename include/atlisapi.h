		...
		__in DWORD dwFlags=ATL_FORM_FLAG_NONE) throw()
	{
		_ATLTRY
		{
			...
			else if (httpMethod == HTTP_METHOD_POST)
			{
				...
				{
					if (strnicmp(szContentType, "multipart/form-data", 19) == 0)
						m_bMultiPart = TRUE;
				...
				if (strnicmp(szContentType, "application/x-www-form-urlencoded", 33) == 0 && !m_pFormVars->IsShared())
					return InitFromPost();

				// If POSTed data is encoded as multipart/form-data, use CMultiPartFormParser.
				if (strnicmp(szContentType, "multipart/form-data", 19) == 0 && !m_pFormVars->IsShared())
				{
					if (m_pFormVars != &m_QueryParams)
						delete m_pFormVars;
					m_pFormVars = NULL;
...
public:
	...
	BOOL SetWriteToClient(__in BOOL bSendOutput) throw()
	{
		...
		if (!bSendOutput)
			AppendHeader("x-http-method", "head");
		return TRUE;
	}
