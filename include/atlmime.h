...
//		BOOL bEncoded = FALSE;
	...
	inline int GetRequiredBufferSize(int nMaxLineLength) throw()
	{
		...
		const static DWORD FROMLINE    = 12;
		const static DWORD TOLINE      = 10;
...
public:
	...
	inline BOOL Initialize(LPCSTR szText, int nTextLen = -1, IMultiLanguage* pMultiLanguage = NULL, UINT uiCodePage = 0) throw()
	{
		BOOL bRet = TRUE;
...
public:
	...
	// uiCodePage - the codepage (optional)
	inline BOOL AddText(LPCSTR szText, int nTextLen = -1, int nPos = 1, UINT uiCodePage = 0) throw()
	{
		if (szText == NULL)
			return FALSE;
